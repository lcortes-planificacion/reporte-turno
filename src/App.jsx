...
const estadoObj = (label) => ESTADOS.find(e => e.label === label) || ESTADOS[3];
...
export default function ReporteTurno() {
  ...
  const addTarea = (actId) => setActividades(p => p.map(a =>
    a.id === actId ? { ...a, tareasRealizadas: [...a.tareasRealizadas, { id: Date.now() + Math.random(), texto: "" }] } : a
  ));
  const updateTarea = (actId, tareaId, texto) => setActividades(p => p.map(a =>
    a.id === actId ? { ...a, tareasRealizadas: a.tareasRealizadas.map(t => t.id === tareaId ? { ...t, texto } : t) } : a
  ));
  const removeTarea = (actId, tareaId) => setActividades(p => p.map(a =>
    a.id === actId ? { ...a, tareasRealizadas: a.tareasRealizadas.filter(t => t.id !== tareaId) } : a
  ));
  ...
  const removeFoto = (actId, idx) => setActividades(p => p.map(a =>
    a.id === actId ? { ...a, fotos: a.fotos.filter((_, i) => i !== idx) } : a
  ));
      ...
      const fotosHTML = a.fotos?.length ? `
        <div style="margin-top:12px;">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:8px;">
            ${a.fotos.map(f => `<img src="${f.dataUrl}" style="width:160px;height:120px;object-fit:cover;border-radius:6px;border:1px solid #E2E8F0;" />`).join("")}
          </div>
        </div>` : "";
      ...
      return `
        <div style="border:1px solid #E2E8F0;border-radius:12px;margin-bottom:24px;overflow:hidden;page-break-inside:avoid;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1px;background:#E2E8F0;">
            ${a.ran ? `<div style="background:#FEF9C3;padding:10px 14px;"><div style="font-size:10px;font-weight:700;color:#92400E;">RAN</div><div style="font-size:13px;font-weight:600;color:#78350F;">${a.ran}</div></div>` : ""}
            ${a.unidad ? `<div style="background:#EFF6FF;padding:10px 14px;"><div style="font-size:10px;font-weight:700;color:#1D4ED8;">UNIDAD / EQUIPO</div><div style="font-size:13px;font-weight:600;color:#1E40AF;">${a.unidad}</div></div>` : ""}
            ${a.cliente ? `<div style="background:#F3E8FF;padding:10px 14px;"><div style="font-size:10px;font-weight:700;color:#6B21A8;">CLIENTE</div><div style="font-size:13px;font-weight:600;color:#581C87;">${a.cliente}</div></div>` : ""}
            ${a.tecnicos ? `<div style="background:#F0FDF4;padding:10px 14px;"><div style="font-size:10px;font-weight:700;color:#166534;">TÉCNICOS</div><div style="font-size:13px;font-weight:600;color:#14532D;">${a.tecnicos}</div></div>` : ""}
            ${a.supervisor ? `<div style="background:#F8FAFC;padding:10px 14px;"><div style="font-size:10px;font-weight:700;color:#475569;">SUPERVISOR</div><div style="font-size:13px;font-weight:600;color:#1E293B;">${a.supervisor}</div></div>` : ""}
          </div>
          ...
          <div style="padding:16px 20px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
              ...
              <span style="background:${est.bg};color:${est.color};border-radius:20px;padding:3px 12px;font-size:12px;font-weight:700;">${a.estado}</span>
            </div>
              ...
              <div style="margin-bottom:12px;">
                ...
                ${tareasOk.map((t, ti) => `
                  <div style="display:flex;gap:8px;margin-bottom:6px;align-items:flex-start;">
                    <span style="background:#E2E8F0;border-radius:50%;width:20px;height:20px;display:inline-flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#475569;flex-shrink:0;">${ti + 1}</span>
                    <span style="font-size:13px;color:#1E293B;line-height:1.4;">${t.texto}</span>
                  </div>`).join("")}
              </div>` : ""}
            ...
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
              ...
              <div style="flex:1;background:#E2E8F0;border-radius:99px;height:10px;overflow:hidden;">
                <div style="height:100%;border-radius:99px;width:${a.avance}%;background:${a.avance === 100 ? "#10B981" : a.avance >= 60 ? "#F59E0B" : "#EF4444"};"></div>
              </div>
              <span style="font-size:14px;font-weight:800;color:#1E293B;min-width:40px;text-align:right;">${a.avance}%</span>
            </div>

            ${a.observaciones ? `<div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:10px 12px;font-size:13px;color:#475569;">📝 ${a.observaciones}</div>` : ""}
            ${fotosHTML}
          </div>
        </div>`;
    }).join("");
            ...
            return (
              <div key={a.id} style={S.previewCard}>
                ...
                <div style={S.previewCardMeta}>
                  ...
                  {a.cliente  && <span style={{ ...S.metaChip, background: "#F3E8FF", color: "#6B21A8" }}>🏢 {a.cliente}</span>}
                  {a.tecnicos && <span style={{ ...S.metaChip, background: "#F0FDF4", color: "#166534" }}>👷 {a.tecnicos}</span>}
                  {a.supervisor && <span style={{ ...S.metaChip, background: "#F8FAFC", color: "#334155" }}>👤 {a.supervisor}</span>}
                </div>

                <div style={S.previewCardBody}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                    <div style={S.sectionLabel}>ACTIVIDAD DEL TURNO</div>
                    <span style={{ ...S.estadoBadge, color: est.color, background: est.bg }}>{a.estado}</span>
                  </div>
                  ...
                  {a.tareasPendientes && (
                    <div style={{ marginBottom: 10 }}>
                      ...
                      <div style={{ fontSize: 13, color: "#B45309", background: "#FFFBEB", borderRadius: 6, padding: "6px 10px" }}>{a.tareasPendientes}</div>
                    </div>
                  )}
                  ...
                  <div style={S.avanceRow}>
                    ...
                    <div style={S.avanceBarWrap}>
                      <div style={S.avanceBarBg}>
                        <div style={{ ...S.avanceBarFill, width: `${a.avance}%`, background: a.avance === 100 ? "#10B981" : a.avance >= 60 ? "#F59E0B" : "#EF4444" }} />
                      </div>
                      <span style={S.avancePct}>{a.avance}%</span>
                    </div>
                  </div>

                  {a.observaciones && <div style={S.obsBox}>📝 {a.observaciones}</div>}
        ...
        {actividades.map((a, i) => (
          <div key={a.id} style={S.section}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
              ...
              <div style={S.fieldGroup}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {TURNOS.map(t => (
                    <button key={t}
                      style={{ ...S.turnoBtn, ...(a.turno === t ? S.turnoBtnActive : {}) }}
            ...
            <div style={S.fieldGroup}>
              ...
              <input style={S.input} placeholder="Nombre del cliente" value={a.cliente}
            ...
            <div style={S.row2}>
              ...
              <div style={S.fieldGroup}>
                ...
                <input style={S.input} placeholder="Nombre del supervisor" value={a.supervisor}
            ...
            <div style={S.fieldGroup}>
              ...
              {a.tareasRealizadas.map((t, ti) => (
                <div key={t.id} style={S.tareaRow}>
                  ...
                  <input style={{ ...S.input, flex: 1 }} placeholder={`Tarea ${ti + 1}...`}
                    value={t.texto} onChange={e => updateTarea(a.id, t.id, e.target.value)} />
                  {a.tareasRealizadas.length > 1 && (
                    <button style={S.tareaRemoveBtn} onClick={() => removeTarea(a.id, t.id)}>✕</button>
                  )}
                </div>
              ))}
              <button style={S.addTareaBtn} onClick={() => addTarea(a.id)}>+ Agregar tarea</button>
            </div>

            <div style={S.fieldGroup}>
              <label style={S.label}>Tareas pendientes</label>
              <textarea style={{ ...S.textarea, borderColor: "#FCD34D" }}
                placeholder="Actividades que quedan para el siguiente turno..."
                value={a.tareasPendientes}
                onChange={e => updateActividad(a.id, "tareasPendientes", e.target.value)} />
            </div>

            <div style={S.row2}>
              <div style={S.fieldGroup}>
                <label style={S.label}>Estado</label>
                <select style={S.select} value={a.estado}
                  onChange={e => updateActividad(a.id, "estado", e.target.value)}>
                  {ESTADOS.map(e => <option key={e.label}>{e.label}</option>)}
                </select>
              </div>
              <div style={S.fieldGroup}>
                <label style={S.label}>Avance: {a.avance}%</label>
                <input type="range" min={0} max={100} step={5} style={S.range} value={a.avance}
                  onChange={e => updateActividad(a.id, "avance", Number(e.target.value))} />
                <div style={S.avanceBarBg}>
                  <div style={{ ...S.avanceBarFill, width: `${a.avance}%`, background: a.avance === 100 ? "#10B981" : a.avance >= 60 ? "#F59E0B" : "#EF4444" }} />
                </div>
              </div>
...
const S = {
  ...
  turnoBtn: { width: 44, height: 38, border: "1.5px solid #E2E8F0", borderRadius: 8, fontSize: 15, fontWeight: 700, background: "#F8FAFC", color: "#64748B", cursor: "pointer" },
  turnoBtnActive: { background: "#1E293B", color: "#fff", borderColor: "#1E293B" },
  ...
  tareaRowNum: { width: 22, height: 22, background: "#E2E8F0", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#475569", flexShrink: 0 },
  tareaRemoveBtn: { background: "none", border: "none", color: "#94A3B8", fontSize: 14, cursor: "pointer", padding: "0 4px", flexShrink: 0 },
  addTareaBtn: { padding: "6px 14px", border: "1.5px dashed #CBD5E1", borderRadius: 6, background: "none", color: "#64748B", fontSize: 13, fontWeight: 600, cursor: "pointer", marginTop: 2 },
  ...
  estadoBadge: { borderRadius: 20, padding: "3px 10px", fontSize: 12, fontWeight: 600 },
  ...
  tareaPreviewItem: { display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 5 },
  tareaNum: { width: 20, height: 20, background: "#E2E8F0", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#475569", flexShrink: 0, marginTop: 1 },
  avanceRow: { display: "flex", alignItems: "center", gap: 8, marginBottom: 10 },
  ...
  avanceBarBg: { height: 8, background: "#E2E8F0", borderRadius: 99, overflow: "hidden", flex: 1 },
  avanceBarFill: { height: "100%", borderRadius: 99, transition: "width 0.3s ease" },
  obsBox: { background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 8, padding: "8px 12px", fontSize: 13, color: "#475569", marginTop: 4 },
  ...
  previewFooter: { textAlign: "center", color: "#94A3B8", fontSize: 12, margin: "8px 0 16px" },
