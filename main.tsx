...
import { pushEntrega, fetchUltimaEntrega, flushQueue, getPendingCount } from "./lib/sync";
...
export default function ReporteTurno() {
  ...
  // ── Conectividad y cola de sincronización ────────────────────────────────
  ...
  const [entregaConfirmada, setEntregaConfirmada] = useState(null); // timestamp string o null

  const intentarSincronizar = useCallback(() => {
    flushQueue((restantes) => setPendingCount(restantes));
  }, []);
  ...
  // ── Entrega de turno (Supabase) ──────────────────────────────────────────
  const cargarTurnoAnterior = async () => {
    const ultima = await fetchUltimaEntrega();
    if (!ultima) {
      alert(supabaseEnabled
        ? "No hay entregas de turno registradas todavía."
        : "La entrega entre turnos no está configurada en esta instalación.");
      return;
    }
    const cuando = new Date(ultima.created_at).toLocaleString("es-CL");
    if (!confirm(`Se encontró una entrega del ${cuando} (Turno ${ultima.turno || "?"}, ${ultima.linea || ""}).\n\n¿Cargar esas actividades? Esto reemplaza lo que tienes escrito ahora en el formulario.`)) {
      return;
    }
    setActividades(ultima.actividades);
    setStep("form");
  };

  const enviarEntrega = async () => {
    if (!firmaSaliente) {
      alert("Falta la firma de quien entrega el turno.");
      return;
    }
    setEnviando(true);
    ...
    const row = {
      ...
      turno: primera.turno || null,
      linea: primera.linea || null,
      ran: primera.ran || null,
      unidad: primera.unidad || null,
      cliente: primera.cliente === "__manual__" ? primera.clienteManual : primera.cliente,
      tecnicos: primera.tecnicos || null,
      supervisor: primera.supervisor === "__manual__" ? primera.supervisorManual : primera.supervisor,
      planificacion: primera.planificacion === "__manual__" ? primera.planificacionManual : primera.planificacion,
      avance_promedio: avancePromedio,
      actividades: actividades.map((a) => ({ ...a, fotos: [] })), // las fotos no viajan a Supabase (peso); quedan solo en el PDF/local
      firma_saliente: firmaSaliente,
      firma_entrante: firmaEntrante,
      nombre_saliente: nombreSaliente || null,
      nombre_entrante: nombreEntrante || null,
      device_info: navigator.userAgent,
    };
    const res = await pushEntrega(row);
    setEnviando(false);
    setPendingCount(getPendingCount());
    if (res.ok) {
      setEntregaConfirmada(new Date().toLocaleString("es-CL"));
    } else if (res.queued) {
      setEntregaConfirmada(new Date().toLocaleString("es-CL") + " (pendiente de sincronizar)");
    } else {
      alert("La entrega de turno entre dispositivos no está configurada todavía. La firma y el reporte quedaron guardados localmente; puedes exportarlos con el botón 💾.");
    }
  };
    ...
    const actividadesHTML = actividades.map((a, i) => {
      ...
      const renderTareasPDF = (lista, titulo, color, resaltar = false) =>
        lista.length
          ? `<div style="margin-bottom:7px;">
              ...
              <div style="${resaltar ? "background:#FFF8ED;border:1px solid #E8A33D;border-radius:6px;padding:5px 7px;" : ""}display:grid;grid-template-columns:1fr 1fr 1fr;gap:1px 8px;">
                ${lista.map((t) => `
                  <div style="font-size:${resaltar ? "10px" : "9px"};font-weight:${resaltar ? "600" : "400"};color:${resaltar ? "#8A5A1E" : "#14181D"};padding:${resaltar ? "2px 0" : "1px 0"};word-break:break-word;white-space:normal;">
      ...
      return `
        <div style="border:1px solid #E2E8F0;border-radius:10px;overflow:hidden;${breakStyle}">
          <div style="background:#14181D;padding:10px 14px;display:flex;align-items:center;gap:10px;">
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:#E2E8F0;">
            ${a.ran ? `<div style="background:#FEF9C3;padding:5px 8px;"><div style="font-size:7px;font-weight:700;color:#8A5A1E;">RAN</div><div style="font-size:10px;font-weight:600;color:#78350F;">${a.ran}</div></div>` : ""}
            ...
            ${supervisorLabel ? `<div style="background:#F8FAFC;padding:5px 8px;"><div style="font-size:7px;font-weight:700;color:#475569;">SUPERVISOR</div><div style="font-size:10px;font-weight:600;color:#14181D;">${supervisorLabel}</div></div>` : ""}
          ...
          <div style="padding:10px 14px;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
              ...
              <div style="flex:1;background:#E2E8F0;border-radius:99px;height:7px;overflow:hidden;">
                <div style="height:100%;border-radius:99px;width:${avance}%;background:${avance === 100 ? "#1E7A4C" : avance >= 60 ? "#C9822E" : "#B3261E"};"></div>
              </div>
              <span style="font-size:12px;font-weight:800;color:#14181D;min-width:34px;text-align:right;">${avance}%</span>
            </div>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  html, body { width: 100%; background: white; font-family: 'Segoe UI', system-ui, sans-serif; color: #14181D; }
...
<body>
  <div style="background:#14181D;padding:12px 16px;border-radius:8px 8px 0 0;display:flex;align-items:center;justify-content:space-between;">
  ...
  <div style="height:3px;background:#3E6B8A;margin-bottom:12px;"></div>
  ${actividadesHTML}
  ...
  const hayAvisos = !online || pendingCount > 0 || deferredPrompt;
  const BarraEstado = hayAvisos ? (
    <div className="banner-offline" style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center", justifyContent: "space-between", background: "#1E242B", padding: "6px 16px", fontSize: 11, color: "#E2E8F0" }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        {!online && <span>📴 Sin conexión — guardando localmente</span>}
        {pendingCount > 0 && <span className={online ? "sync-dot-pending" : ""}>⏳ {pendingCount} pendiente{pendingCount !== 1 ? "s" : ""} por sincronizar</span>}
      </div>
      {deferredPrompt && (
        <button onClick={instalarApp} style={{ background: "#E8A33D", border: "none", color: "#14181D", fontWeight: 700, fontSize: 11, borderRadius: 6, padding: "4px 10px", cursor: "pointer" }}>
          ⬇️ Instalar app
        </button>
      )}
    </div>
    ...
    return (
      <div style={S.root}>
        ...
        <div style={S.container}>
          <div className="plate" style={S.topBar}>
          ...
          <div style={S.shareBar}>
            <button style={{ ...S.shareBtn, background: "#14181D", color: "#fff", border: "none" }} onClick={handleExportPDF}>
              📄 Exportar PDF
            </button>
          ...
          {actividades.map((a, i) => {
            ...
            return (
              <div key={a.id} style={S.previewCard}>
                <div className="plate" style={S.previewCardTopBar}>
                  <div style={{ fontSize: 20 }}>⚙️</div>
                  <div>
                    <div style={S.previewCardBarTitle}>Turno {a.turno} — {a.linea}{nroLinea} — Actividad {i + 1}</div>
                    <div className="mono" style={S.previewCardBarSub}>{fecha}</div>
                  </div>
                </div>
                <div style={S.previewCardMeta}>
                  {a.ran && <span className="mono" style={{ ...S.metaChip, background: "#FFF3DF", color: "#8A5A1E" }}>📋 RAN: {a.ran}</span>}
                ...
                <div style={S.previewCardBody}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                    ...
                    <div style={{ flex: 1, background: "#E2E8F0", borderRadius: 99, height: 8, overflow: "hidden" }}>
                      <div style={{ height: "100%", borderRadius: 99, width: `${avance}%`, background: avance === 100 ? "#1E7A4C" : avance >= 60 ? "#C9822E" : "#B3261E" }} />
                    </div>
                  ...
                  {a.tareas.map((t) => (
                    ...
                    <div key={t.id} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "5px 0", borderBottom: "1px solid #F1F5F9" }}>
                      <span style={{
                        ...
                        background: t.estado === "finalizado" ? "#DFF3E7" : t.estado === "noaplica" ? "#F1F5F9" : "#FFF3DF",
                        color: t.estado === "finalizado" ? "#1E7A4C" : t.estado === "noaplica" ? "#94A3B8" : "#8A5A1E",
                      }}>
                        {t.estado === "finalizado" ? "✅" : t.estado === "noaplica" ? "N/A" : "⏳"}
                      </span>
                      <div style={{ flex: 1 }}>
                        <span style={{ fontSize: 13, color: t.estado === "noaplica" ? "#94A3B8" : "#14181D" }}>{t.nombre}</span>
          ...
          <div className="plate" style={S.topBar}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                <div style={S.topBarSub}>{actividades.length} actividad{actividades.length !== 1 ? "es" : ""} · {avancePromedio}% avance promedio</div>
              </div>
            </div>
          </div>

          <button style={{ ...S.shareBtn, marginBottom: 14 }} onClick={() => setStep("preview")}>← Volver al resumen</button>

          {entregaConfirmada ? (
            <div className="plate" style={{ borderRadius: 12, padding: "22px 18px", textAlign: "center", marginBottom: 16 }}>
              <div style={{ fontSize: 28, marginBottom: 6 }}>✅</div>
              <div style={{ color: "#F1F5F9", fontWeight: 800, fontSize: 15 }}>Entrega registrada</div>
              <div className="mono" style={{ color: "#E8A33D", fontSize: 12, marginTop: 6 }}>{entregaConfirmada}</div>
              <div style={{ color: "#94A3B8", fontSize: 12, marginTop: 10 }}>
                {nombreSaliente && <>De <strong style={{ color: "#E2E8F0" }}>{nombreSaliente}</strong> </>}
                {nombreEntrante && <>a <strong style={{ color: "#E2E8F0" }}>{nombreEntrante}</strong></>}
              </div>
              <button
                style={{ marginTop: 14, background: "none", border: "1px solid #475569", color: "#94A3B8", borderRadius: 8, padding: "7px 14px", fontSize: 12, cursor: "pointer" }}
                onClick={() => setEntregaConfirmada(null)}
              >
            ...
            <>
              {!supabaseEnabled && (
                <div style={{ background: "#FFF3DF", border: "1px solid #F3D19C", borderRadius: 8, padding: "10px 12px", fontSize: 12, color: "#7A4A0E", marginBottom: 14 }}>
                  ℹ️ La sincronización entre dispositivos aún no está configurada en esta instalación. Puedes firmar igual: la firma queda incluida al exportar PDF o JSON (💾).
                </div>
              )}

              <div style={S.section}>
                <div style={S.sectionLabel}>QUIEN ENTREGA EL TURNO</div>
                <div style={S.fieldGroup}>
                  <label style={S.label}>Nombre</label>
                  <input style={S.input} placeholder="Nombre de quien entrega" value={nombreSaliente}
                    onChange={(e) => setNombreSaliente(e.target.value)} />
                </div>
                <SignaturePad value={firmaSaliente} onChange={setFirmaSaliente} label="Firmar aquí para entregar el turno" />
              </div>

              <div style={S.section}>
                <div style={S.sectionLabel}>QUIEN RECIBE EL TURNO <span style={{ opacity: 0.6, fontWeight: 400, textTransform: "none" }}>(opcional)</span></div>
                <div style={S.fieldGroup}>
                  <label style={S.label}>Nombre</label>
                  <input style={S.input} placeholder="Nombre de quien recibe" value={nombreEntrante}
                    onChange={(e) => setNombreEntrante(e.target.value)} />
                </div>
                <SignaturePad value={firmaEntrante} onChange={setFirmaEntrante} label="Firmar aquí para confirmar recepción" />
              </div>

              <button
                style={{ ...S.btnPrimary, background: firmaSaliente ? "#C9822E" : "#CBD5E1", cursor: firmaSaliente && !enviando ? "pointer" : "default" }}
                disabled={!firmaSaliente || enviando}
                onClick={enviarEntrega}
              >
                {enviando ? "Enviando..." : "Confirmar entrega de turno"}
              </button>
            </>
          )}
        </div>
    ...
    <div style={S.root}>
      ...
      <div style={S.container}>
        <div className="plate" style={S.topBar}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              ...
              {supabaseEnabled && (
                <button onClick={cargarTurnoAnterior} style={S.clearBtn} title="Cargar entrega del turno anterior">⬇️</button>
              )}
        ...
        {actividades.map((a, i) => {
          ...
          const clienteLabel = a.cliente === "__manual__" ? a.clienteManual : a.cliente;
          const nroLinea = a.nroLinea ? ` N°${a.nroLinea}` : "";
          const lineaColor = a.linea === "Ensamble" ? "#3E6B8A" : "#A85C32";
          const avanceColor = avance === 100 ? "#1E7A4C" : avance >= 60 ? "#C9822E" : "#B3261E";
          return (
            <div key={a.id} style={{ background:"#fff", borderRadius:10, marginBottom:6, border:"1px solid #E2E8F0", overflow:"hidden" }}>
              ...
              <div style={{ display:"flex", alignItems:"stretch" }}>
                ...
                <div onClick={()=>setActividadAbierta(abierta?null:a.id)} style={{ flex:1, display:"flex", alignItems:"center", gap:8, padding:"9px 11px", cursor:"pointer", background:abierta?"#F1F5F9":"#fff", userSelect:"none", minWidth:0 }}>
                  <div style={{ background:abierta?"#14181D":"#E2E8F0", color:abierta?"#fff":"#64748B", borderRadius:5, width:22, height:22, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700 }}>{i+1}</div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:4, flexWrap:"wrap" }}>
                      ...
                      {a.ran && <span className="mono" style={{ fontSize:12, fontWeight:700, color:"#14181D" }}>RAN {a.ran}</span>}
                  ...
                  <div style={{ display:"flex", gap:4, flex:1 }}>
                    {LINEAS.map(l => (
                      <button key={l} onClick={() => cambiarLinea(a.id,l)} style={{
                        ...
                        background: a.linea===l?(l==="Ensamble"?"#3E6B8A":"#A85C32"):"#F8FAFC",
                        color: a.linea===l?"#fff":"#64748B",
                        borderColor: a.linea===l?(l==="Ensamble"?"#3E6B8A":"#A85C32"):"#E2E8F0",
                  ...
                  <div style={{ display:"flex", gap:4, flex:1 }}>
                    {TURNOS.map(t => (
                      <button key={t} onClick={() => updateActividad(a.id,"turno",t)} style={{
                        ...
                        background: a.turno===t?"#14181D":"#F8FAFC",
                        color: a.turno===t?"#fff":"#64748B",
                        borderColor: a.turno===t?"#14181D":"#E2E8F0",
                      }}>T{t}</button>
                    ))}
                  </div>
                </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                <div style={{
                  ...
                  color: avance === 100 ? "#1E7A4C" : avance >= 60 ? "#C9822E" : "#64748B"
                }}>
                  {avance}%
                </div>
              </div>
              ...
              <div style={{ background: "#E2E8F0", borderRadius: 99, height: 8, overflow: "hidden", marginBottom: 14 }}>
                <div style={{
                  ...
                  width: `${avance}%`,
                  background: avance === 100 ? "#1E7A4C" : avance >= 60 ? "#C9822E" : "#B3261E"
                }} />
              </div>
              ...
              {(() => {
                ...
                <div key={t.id} style={{ padding:"4px 0", borderBottom:"1px solid #F1F5F9" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    <span style={{ flex:1, fontSize:12, color:"#14181D", lineHeight:1.3 }}>{t.nombre}</span>
                    <div style={{ display:"flex", gap:3, flexShrink:0 }}>
                      {[
                        { val:"finalizado", label:"✓",   colorOn:"#1E7A4C", bgOn:"#DFF3E7" },
                        { val:"noaplica",   label:"N/A", colorOn:"#94A3B8", bgOn:"#F1F5F9" },
                        { val:"pendiente",  label:"⏳",  colorOn:"#C9822E", bgOn:"#FFF3DF" },
                      ].map(op => (
                        <button key={op.val}
                          style={{
                  ...
                  {t.estado === "pendiente" && (
                    <input
                      style={{ width:"100%", marginTop:4, padding:"5px 8px", border:"1px solid #E8A33D", borderRadius:5, fontSize:11, color:"#8A5A1E", background:"#FFF8ED", boxSizing:"border-box", outline:"none" }}
...
const S = {
  ...
  topBar: { margin: "0 -16px 24px", padding: "18px 20px" },
  ...
  input: { width: "100%", padding: "9px 12px", border: "1.5px solid #E2E8F0", borderRadius: 8, fontSize: 14, color: "#14181D", background: "#F8FAFC", boxSizing: "border-box", outline: "none" },
  textarea: { width: "100%", padding: "9px 12px", border: "1.5px solid #E2E8F0", borderRadius: 8, fontSize: 14, color: "#14181D", background: "#F8FAFC", boxSizing: "border-box", minHeight: 72, resize: "vertical", outline: "none", fontFamily: "inherit" },
  select: { width: "100%", padding: "9px 12px", border: "1.5px solid #E2E8F0", borderRadius: 8, fontSize: 14, color: "#14181D", background: "#F8FAFC", boxSizing: "border-box" },
  actCardNum: { background: "#14181D", color: "#fff", borderRadius: 6, width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, flexShrink: 0 },
  ...
  tareaHeader: { fontSize: 11, fontWeight: 800, letterSpacing: "0.06em", color: "#14181D", background: "#E2E8F0", padding: "6px 10px", borderRadius: 6, marginBottom: 6, marginTop: 10, textTransform: "uppercase" },
  ...
  btnPrimary: { width: "100%", padding: "14px", background: "#14181D", color: "#fff", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: "pointer", letterSpacing: "-0.2px" },
  shareBar: { display: "flex", gap: 8, margin: "16px 0 20px" },
  shareBtn: { flex: 1, padding: "11px 12px", border: "1.5px solid #E2E8F0", borderRadius: 8, background: "#fff", color: "#14181D", fontSize: 14, fontWeight: 600, cursor: "pointer" },
  ...
  previewCardTopBar: { padding: "14px 18px", display: "flex", alignItems: "center", gap: 12 },
  ...
  avancePct: { fontSize: 13, fontWeight: 700, color: "#14181D", minWidth: 38, textAlign: "right" },
